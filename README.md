...
| `USHELF_APP_PASSWORD`  | unset                   | Optional password for all HTTP library content                   |
...
The native CLI's `--home` flag overrides `USHELF_ROOT`. It also accepts `USHELF_IMAGE`. Use `ushelf config show` for effective values and `ushelf config set` for persistent host, port, base-path, image, or app password overrides. The password is prompted without echo and stored in the private `secrets/app-password` file, separate from `config.json`; `config show` reports only `configured` or `unset`.

After `ushelf config set app-password`, run `ushelf start` to apply it to a running service. Password changes and `ushelf config unset app-password` likewise take effect after `ushelf start`; the CLI recreates the container when the saved value changes. A nonempty `USHELF_APP_PASSWORD` overrides the saved value and is passed to the container without putting its value on the Docker command line. Keep an environment override set on subsequent starts if you use one. Do not place the password in a shell history command or commit it to `.env` in Git.
