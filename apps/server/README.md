...
- A nonempty `USHELF_WEB_PASSWORD` or a saved `<USHELF_SECRETS_DIR>/web-password` optionally requires sign-in for HTTP library routes. The environment value takes precedence. With neither configured, access is password-free. Protected production access requires an HTTPS proxy because session cookies are Secure.
