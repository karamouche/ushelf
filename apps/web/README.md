...
- `src/App.tsx` contains the library, reader, setup, login, recovery, OAuth consent, device approval, and connection-management routes; `src/api.ts` is the API client.
...
- In development, Vite runs on `127.0.0.1:43111` and proxies the configured base path's `/api` endpoint to the server on port `43110`.
- In production, the server serves the generated `dist/` directory and handles SPA fallback.
- Remote mode protects the reader and API with a single-owner session. `/connections` displays the MCP endpoint and grants; local mode remains unchanged.
